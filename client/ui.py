...
class RegisterFrame(customtkinter.CTkFrame):
    ...
    def register_user(self):
        """
            Used to take all the credentials entered by the user and send to the client.RegisterDataSend Func 
        """
        username = self.username_entry.get()
...
class AfterLoginFrame(customtkinter.CTkFrame):
    def __init__(self, master, switch_frame_callback):
        ...
        self.profile_button = customtkinter.CTkButton(self, image=profile_icon, width=0, text="", fg_color="#444444", command=self.switch_to_EditProfileFrame)
        ...
        # Text widget and Scrollbar directly inside the frame
    ...
    def send_normal_message(self):
        ...
        check = client.send_normal_message(message)
        if check == SUCCESS:
            pass
        elif check == FAILED:
            messagebox.showerror("Error", "Maybe server went offline/You been kicked out")
    ...
    def search(self):
        ...
        if check_search == SUCCESS:
            ...
            print(f"{username}")
            print("No user found")
    def switch_to_EditProfileFrame(self):
        # Call the callback to switch to EditProfile
        self.switch_frame_callback("EditProfileFrame")
        logging.info("Switched to EditProfileFrame")

class EditProfileFrame(customtkinter.CTkFrame):
    def __init__(self, master, switch_frame_callback):
        super().__init__(master)
        self.switch_frame_callback = switch_frame_callback
        ...
        # Back icon as button
        self.back_button = customtkinter.CTkButton(self, image=back_icon,text="", border_width=0,fg_color="#333333",width=0, command=self.switch_to_AfterLoginFrame)
        self.back_button.image = back_icon
        self.back_button.grid(row=0, column=0, pady=(10, 0), sticky="w")

        # Profile pic of the User
        profile_image = Image.open("rc/unkown1.png")
        profile_image = profile_image.resize((200,200))
        profile_icon = ImageTk.PhotoImage(profile_image)

        # Profile pic as a button
        self.profile_button = customtkinter.CTkButton(self, image=profile_icon, width=0, text="", fg_color="#444444")
        self.profile_button.place(relx=0.86, rely=0.2, anchor="ne")

        # Label of the Username
        self.username_label = customtkinter.CTkLabel(self, text="Username:")
        self.username_label.grid(row=4, column=0, pady=(50,0), padx=10, sticky="w")

        # Entry for the Username of the user
        self.username_entry = customtkinter.CTkEntry(self)
        self.username_entry.grid(row=4,column=1, pady=(50,0), padx=10, sticky="e")

        # Label of the Name
        self.name_label = customtkinter.CTkLabel(self, text="Name:")
        self.name_label.grid(row=5, column=0, pady=(14,10), padx=10, sticky="w")

        # Entry for the Name of the user
        self.name_entry = customtkinter.CTkEntry(self)
        self.name_entry.grid(row=5,column=1, pady=(14,10), padx=10, sticky="e")

        # Label of the DOB
        self.dob_label = customtkinter.CTkLabel(self, text="Date of Birth:")
        self.dob_label.grid(row=6, column=0, pady=(6,10), padx=10, sticky="w")

        # Entry for dob of the user
        self.dob_entry = customtkinter.CTkEntry(self, placeholder_text="DD/MM/YY")
        self.dob_entry.grid(row=6, column=1, pady=(6, 10), padx=10, sticky="e")

        # Label of Email
        self.email_label = customtkinter.CTkLabel(self, text="Email:")
        self.email_label.grid(row=7, column=0, pady=(6, 10), padx=10, sticky="w")

        # Entry for email of the User
        self.email_entry = customtkinter.CTkEntry(self, placeholder_text="example@gmail.com")
        self.email_entry.grid(row=7, column=1, pady=(6, 10), padx=10, sticky="e")

        # Save button
        self.save_button = customtkinter.CTkButton(self, text="Save" )
        self.save_button.place(relx=0.439, rely=0.68, anchor="ne")

    def switch_to_AfterLoginFrame(self):
        # Call the callback to switch back to the AfterLogin
        self.switch_frame_callback("AfterLoginFrame")
        logging.info("Switched to AfterLogin")
...
class App(customtkinter.CTk):
    ...
    def show_frame(self, frame_name):
        ...
        elif frame_name == "EditProfileFrame":
            self.current_frame = EditProfileFrame(self, self.show_frame)
